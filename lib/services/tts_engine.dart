...
const List<String> ttsLangs = [
  'en', 'ko', 'ja', 'ar', 'bg', 'cs', 'da', 'de', 'el', 'es', 'et', 'fi',
  'fr', 'hi', 'hr', 'hu', 'id', 'it', 'lt', 'lv', 'nl', 'pl', 'pt', 'ro',
  'ru', 'sk', 'sl', 'sv', 'tr', 'uk', 'vi',
];

const Map<String, String> ttsLangNames = {
  'en': 'English', 'ko': '한국어', 'ja': '日本語', 'ar': 'العربية',
  'bg': 'Български', 'cs': 'Čeština', 'da': 'Dansk', 'de': 'Deutsch',
  'el': 'Ελληνικά', 'es': 'Español', 'et': 'Eesti', 'fi': 'Suomi',
  'fr': 'Français', 'hi': 'हिन्दी', 'hr': 'Hrvatski', 'hu': 'Magyar',
  'id': 'Bahasa Indonesia', 'it': 'Italiano', 'lt': 'Lietuvių', 'lv': 'Latviešu',
...
const List<String> voiceStyles = ['M1', 'M2', 'M3', 'M4', 'M5', 'F1', 'F2', 'F3', 'F4', 'F5'];

const String _hfRepo = 'Supertone/supertonic-3';
const List<String> _onnxModels = [
  'duration_predictor.onnx',
  'text_encoder.onnx',
  'vector_estimator.onnx',
  'vocoder.onnx',
];
const List<String> _jsonFiles = [
  'onnx/tts.json',
...
List<int> _decomposeHangulSyllable(int codePoint) {
  ...
  final result = <int>[_leadingJamoBase + leadingIndex, _vowelJamoBase + vowelIndex];
...
typedef TtsProgressCallback = void Function(double progress, String status);

class TtsDownloader {
  static Future<String> get _ttsDir async {
    final dir = await getApplicationDocumentsDirectory();
    final ttsDir = Directory('${dir.path}/tts');
    if (!ttsDir.existsSync()) await ttsDir.create(recursive: true);
    return ttsDir.path;
  }

  static Future<bool> isDownloaded() async {
    final dir = await _ttsDir;
    for (final model in _onnxModels) {
      if (!File('$dir/onnx/$model').existsSync()) return false;
    }
    return true;
  }

  static Future<String> ensureDownloaded(TtsProgressCallback onProgress) async {
    final dir = await _ttsDir;
    final onnxDir = Directory('$dir/onnx');
    final stylesDir = Directory('$dir/voice_styles');
    if (!onnxDir.existsSync()) await onnxDir.create(recursive: true);
    if (!stylesDir.existsSync()) await stylesDir.create(recursive: true);

    final allFiles = <String>[
      for (final m in _onnxModels) 'onnx/$m',
      ..._jsonFiles,
      for (final v in voiceStyles) 'voice_styles/$v.json',
    ];
    ...
    for (final file in allFiles) {
      ...
      if (localFile.existsSync() && localFile.lengthSync() > 100) {
        ...
        onProgress(done / total, 'Проверка $file');
        continue;
      }

      final url = 'https://huggingface.co/$_hfRepo/resolve/main/$file';
      onProgress(done / total, 'Скачивание $file');
      ...
      onProgress(done / total, 'Скачано $file');
    }
...
class UnicodeProcessor {
  ...
  static Future<UnicodeProcessor> load(String path) async {
    ...
    final indexer = json is List
        ? {for (var i = 0; i < json.length; i++) if (json[i] is int && json[i] >= 0) i: json[i] as int}
  ...
  List<List<List<double>>> _lengthToMask(List<int> lengths, [int? maxLen]) {
    ...
    return lengths.map((len) => [List.generate(maxLen!, (i) => i < len ? 1.0 : 0.0)]).toList();
  }
}
...
class TextToSpeech {
  ...
  Future<Map<String, dynamic>> call(
    ...
    {double speed = 1.05, double silenceDuration = 0.3}
  ) async {
  ...
  Future<Map<String, dynamic>> _infer(
    ...
    final textMask = textMaskRaw is List<List<List<double>>>
        ? textMaskRaw
        : (textMaskRaw as List).map((batch) => (batch as List).map((row) => (row as List).cast<double>()).toList()).toList();
    ...
    var noisyLatent = (latentData['noisyLatent'] as List)
        .map((batch) => (batch as List).map((row) => (row as List).cast<double>()).toList()).toList();
    final latentMask = (latentData['latentMask'] as List)
        .map((batch) => (batch as List).map((row) => (row as List).cast<double>()).toList()).toList();
  ...
  Map<String, dynamic> _sampleNoisyLatent(List<double> duration) {
    ...
    final noisyLatent = List.generate(
      duration.length,
      (_) => List.generate(latentDim, (_) => List.generate(latentLen, (_) {
  ...
  List<T> _safeCast<T>(dynamic raw) {
    ...
    if (raw is List) {
      if (raw.isNotEmpty && raw.first is List) return _flattenList<T>(raw);
      if (T == double) return raw.map((e) => e is num ? e.toDouble() : double.parse(e.toString())).toList() as List<T>;
      return raw.cast<T>();
    }
    throw Exception('Cannot convert to List<$T>');
  }

  List<T> _flattenList<T>(dynamic list) {
    if (list is List) return list.expand((e) => _flattenList<T>(e)).toList();
  ...
  Future<OrtValue> _toTensor(dynamic array, List<int> dims) async {
    ...
    return OrtValue.fromList(Float32List.fromList(flat), dims);
  }

  Future<OrtValue> _scalarToTensor(List<double> array, List<int> dims) async {
    return OrtValue.fromList(Float32List.fromList(array), dims);
  }

  Future<OrtValue> _intToTensor(List<List<int>> array, List<int> dims) async {
    final flat = array.expand((row) => row).toList();
    return OrtValue.fromList(Int64List.fromList(flat), dims);
  }
}

Future<TextToSpeech> loadTextToSpeech(String modelDir) async {
  final onnxDir = '$modelDir/onnx';
  ...
  return TextToSpeech(cfgs, textProcessor, sessions[0], sessions[1], sessions[2], sessions[3]);
}

Future<Style> loadVoiceStyle(String modelDir, String voiceName) async {
  final path = '$modelDir/voice_styles/$voiceName.json';
  ...
  return Style(
    await OrtValue.fromList(Float32List.fromList(ttlData), ttlDims),
    await OrtValue.fromList(Float32List.fromList(dpData), dpDims),
    ttlDims,
    dpDims,
  );
}
...
void writeWavFile(String filename, List<double> audioData, int sampleRate) {
  ...
  for (var byte in [0x52, 0x49, 0x46, 0x46]) buffer.setUint8(offset++, byte);
  buffer.setUint32(offset, 36 + dataSize, Endian.little); offset += 4;
  for (var byte in [0x57, 0x41, 0x56, 0x45]) buffer.setUint8(offset++, byte);
  for (var byte in [0x66, 0x6D, 0x74, 0x20]) buffer.setUint8(offset++, byte);
  ...
  buffer.setUint32(offset, sampleRate * numChannels * 2, Endian.little); offset += 4;
  buffer.setUint16(offset, numChannels * 2, Endian.little); offset += 2;
  buffer.setUint16(offset, bitsPerSample, Endian.little); offset += 2;
  for (var byte in [0x64, 0x61, 0x74, 0x61]) buffer.setUint8(offset++, byte);
